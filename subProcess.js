...
class vy2subProcess{
    ...
    run = ( cmd ) => {
        ...
        sp.stdout.on( 'data', d =>{
            if( this.opts.onMsg ) this.opts.onMsg( d.toString() );
            else this.dumpMsg( 'stdout.data', d.toString() );
        });

        sp.stderr.on( 'data', d =>{
            if( this.opts.onMsg ) this.opts.onMsg( d.toString() );
            else this.dumpMsg( 'stderr.data', d.toString() );
        });
        ...
        sp.on('close', exitCode => {
            ...
            if( this.opts.onClose ) this.opts.onClose( exitCode );
            else this.dumpMsg( 'close (exitCode)', exitCode );
        });
